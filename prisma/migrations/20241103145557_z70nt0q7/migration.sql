/*
  Warnings:

  - You are about to drop the column `avatar` on the `Notifications` table. All the data in the column will be lost.
  - Added the required column `contextType` to the `Notifications` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Notifications" DROP COLUMN "avatar",
ADD COLUMN     "contextId" INTEGER,
ADD COLUMN     "contextType" TEXT NOT NULL;
